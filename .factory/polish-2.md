...
| --- | --- | --- |
| F-1-1 | Kept the shared compact header, legal links, demo banner, Reset demo, Start for real action, and footer on both `/?demo=1` and `/demo`. | `demo keeps the shared home, legal navigation, and footer at its direct entry point`; [local mobile demo](polish-artifacts-2/local-demo-mobile.png). |
...
| F-2-3 | Replaced jargon with bundled versions, renders, and side-by-side comparison; removed every numbered decorative eyebrow from the landing and editor; rewrote README language; updated metadata and the catalog sentence. | `copy-audit.md`; `accessibility baseline for /`; [local desktop screenshot](polish-artifacts-2/local-root-desktop.png). |
| F-2-4 | Rewrote the designed 404 to say “Page not found” and “This page does not exist. Return to the home page.” while retaining the broken-neon sign and recovery action. | `404 names the missing page plainly`; [local 404 screenshot](polish-artifacts-2/local-404-mobile.png). |
...
The live recheck is recorded after the static deployment completes in this same work order.
