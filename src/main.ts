...
let unmountRoute: (() => void) | undefined;

const routes: Record<string, { title: string; description: string }> = {
  '/': { title: 'Diagram Source Studio — check diagram renders', description: 'Compare Mermaid and D2 renders before you commit, then export files that keep their source.' },
...
function footer(): string {
  return `<footer class="site-footer"><p><strong>Diagram Source Studio</strong><br>Check diagram renders before you commit.</p><nav aria-label="Footer navigation"><a href="/privacy" data-link>Privacy</a><a href="/terms" data-link>Terms</a><a href="https://sociobot.in" rel="external">Built by Param Factory <span class="sr-only">(external site)</span></a></nav><p class="build">v0.1.13 · Original generated artwork</p></footer>`;
}
...
function home(): string {
  ...
  <mark>previous -. missing .-> check</mark></pre></div><div class="mini-canvas"><svg viewBox="0 0 620 300" role="img" aria-label="Two renderer paths meet at an inspection node, while a magenta path is broken."><defs><filter id="glow"><feGaussianBlur stdDeviation="3" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs><g class="diagram-lines" fill="none" stroke-width="4" filter="url(#glow)"><path stroke="#36f1e4" d="M120 150H240M380 85H495M120 150C170 150 180 85 240 85"/><path stroke="#ff4fa3" stroke-dasharray="22 14" d="M120 150C170 150 180 225 240 225M380 225H430"/></g><g fill="#19202a" stroke="#36f1e4" stroke-width="2"><rect x="20" y="115" width="100" height="70"/><rect x="240" y="50" width="140" height="70"/><rect x="240" y="190" width="140" height="70"/><rect x="495" y="115" width="105" height="70"/></g><g fill="#f5f1e8" font-size="17" text-anchor="middle"><text x="70" y="157">source</text><text x="310" y="92">11.17.2</text><text x="310" y="232">10.9.8</text><text x="548" y="157">inspect</text></g><circle cx="459" cy="225" r="10" fill="#ff4fa3"/></svg><span class="canvas-status"><i></i> 1 output difference</span></div></div></section>
    <section class="walkthrough" aria-labelledby="how-title"><div class="section-intro"><h2 id="how-title">Check a diagram in three steps</h2></div><ol><li><div class="step-frame source-frame" aria-hidden="true"><span>architecture.mmd</span><code>service --&gt; database</code></div><h3>Open the source</h3><p>Choose a Mermaid or D2 file from your repository.</p></li><li><div class="step-frame compare-frame" aria-hidden="true"><span>11.17.2 ✓</span><span>10.9.8 !</span></div><h3>Compare the renders</h3><p>Place two bundled Mermaid versions side by side.</p></li><li><div class="step-frame export-frame" aria-hidden="true"><span>&lt;metadata&gt;</span><b>SVG</b><b>PNG</b></div><h3>Export an editable file</h3><p>Save SVG or PNG renders with the source embedded.</p></li></ol></section>
    <section class="boundary" aria-labelledby="boundary-title"><div><h2 id="boundary-title">Privacy and product limits</h2></div><div><p>No sign-in is required to use the editor. Diagram source stays on your device.</p><p>Purchase availability checks the public catalog once. License checks send only the token you enter.</p></div></section>
    <section class="pricing" id="pricing" aria-labelledby="price-title"><div><h2 id="price-title">Compare two Mermaid versions</h2><p>The free editor previews, reports syntax problems, and exports diagrams. Studio adds a side-by-side comparison.</p></div><div class="price-plate"><span>Studio comparison</span><strong>$39 once</strong><small>One-time license. After payment, checkout returns here and verifies your Studio access.</small><div id="checkout-action" aria-live="polite"><span>Checking purchase availability…</span></div><a href="/#downloads">Already have a Studio license? Download the app to paste it.</a><p>Studio checkout is hosted by Dodo Payments. Refunds revoke the license.</p></div></section>
    <section class="downloads" id="downloads" aria-labelledby="download-title"><div><h2 id="download-title">Download the desktop app</h2><p>Desktop builds are unsigned until the project owner adds signing certificates.</p></div><div id="download-panel" class="download-panel" aria-live="polite"><span class="tube-dots" aria-hidden="true"><i></i><i></i><i></i></span><p>Checking the latest release…</p></div></section>
  </main>${footer()}`;
}
...
function terms(): string {
  ...
  <h2>No warranty</h2><p>The software is provided without warranty. Review important renders before publishing them.</p></main>${footer()}`;
}

function notFound(): string {
  return `${header()}<main id="main" class="not-found" tabindex="-1"><div class="broken-sign" aria-hidden="true"><span>4</span><i></i><span>4</span></div><h1 tabindex="-1">Page not found</h1><p>This page does not exist. Return to the home page.</p><a class="primary" href="/" data-link>Return home</a></main>${footer()}`;
}
...
function renderRoute(path = location.pathname, push = false, moveFocus = false, search = location.search, hash = location.hash) {
  ...
  const canonical = `https://diagram-source-studio.sociobot.in${route === '/404' ? path : route}`;
